 ...
 *  is not a real node (e.g. the root's incoming stream); the particle shader
 *  ramps its upstream end in so it dissolves into the background. */
...
function straightControls(
  from: THREE.Vector3 | undefined,
  to: THREE.Vector3 | undefined,
): Controls {
  const a = from ?? to ?? new THREE.Vector3();
  const b = to ?? from ?? new THREE.Vector3();
  return [
    a.clone(),
    a.clone().lerp(b, 1 / 3),
    a.clone().lerp(b, 2 / 3),
...
export class SceneProjection {
  ...
  sync(visible: NormalizedScene): boolean {
  ...
  build(focusId: string): SceneProjectionBuilt {
    ...
    edgeEntries.forEach((e, i) => {
      ...
      this.edgeFade.data[p4 + 1] = e.edge.isStub ? 1 : 0;
